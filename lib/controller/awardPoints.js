...
function awardPoints(originalMessage) {
  ...
  if (originalMessage.text.toLowerCase().indexOf('gryffindor') > -1) {
    ...
    this.getPointsFromDatabase.call(this, originalMessage, 'gryffindor', this.slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    this.getPointsFromDatabase.call(this, originalMessage, 'hufflepuff', this.slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    this.getPointsFromDatabase.call(this, originalMessage, 'ravenclaw', this.slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    this.getPointsFromDatabase.call(this, originalMessage, 'slytherin', this.slackBot.awardPointsCallback);
