...
class Dumbledore extends Bot {
  constructor(settings) {
    ...
    this.db = null;   //path로 가서 db 가져옴
    this.user = null; //_loadBotUser를 통해 bot1을 가져옴
    this.params = {
      link_names: 1
    };
  }
  ...
  _loadBotUser() {
    const self = this;
    self.user = this.users.filter(function (user) { //bot1과 같은 이름의 bot user를 가져온다
  ...
  _welcomeMessageManual(originalMessage, bot) {
    var bot = bot;
//    console.log("_welcomeMessageManual : ");
...
//    console.log(message);
    // 메세지에 대한 조건
    if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromDumbledore(message) && !this._isFromSlackbot(message)) {
        ...
        if (this._isAwardingPoints(message)) {
            this._awardPoints(message);  //점수 부여
        } else if (this._isDeductingPoints(message)) {
            this._deductPoints(message); //점수 감점
        } else if (this._isMentioningDumbledore(message)) {
            this._replyWithDumbledore(message); //덤블도어 or professor 언급시
...
//    console.log('boolean: ' + Boolean(message.text));
  ...
  _announcePlainString(orignalMessage, message, bot) {
    var bot = bot;
//    console.log(bot._getChannelById(originalMessage.channel).name);
//    console.log(message);
